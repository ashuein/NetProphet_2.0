generate_lasso_net = function(p_in_target
  ...
  # | 2. prepare_data.R where functions for creating allowed/perturbed matrices, and                | #
  # | scaling/normalizing the expression matrices                                                   | #
  # ================================================================================================= #
  source(paste(p_src_code, "SRC/NetProphet1/global.lars.regulators.r", sep=""))  # for generating lasso network
  source(paste(p_src_code, "prepare_data.R", sep=""))  # for preparing data
  source(paste(p_src_code, "helper.R", sep=""))  # for helper functions
  ...
  # ================================================================================================= #
   ...
   df_allowed_perturbed = generate_allowed_perturbed_matrices(l_in_target, l_in_reg, l_in_sample, NULL)
   ...
   # ------------------------------------------------------------------------------ #
       ...
       df_allowed_perturbed_minus_reg = generate_allowed_perturbed_matrices(l_in_target, l_in_reg[-idx_reg_greenfield], l_in_sample, NULL)
...
if (sys.nframe() == 0){
  ...
  p_src_code = make_option(c("--p_src_code"), type="character", default="/scratch/mblab/dabid/netprophet/code_NetProphet_2.0/", help="path for netpropohet source code")
  flag_microarray = make_option(c("--flag_microarray"), type="character", default="ON", help="ON or OFF for microarray or RNA-Seq data")
  ...
  opt_parser = OptionParser(option_list=list(p_in_target, p_in_reg, p_in_sample, p_in_expr_target, p_in_expr_reg
                                             ...
                                             , greenfield_method, p_out_dir, fname_lasso, flag_debug, flag_parallel
  ...
  if (opt$flag_debug == "ON"){
    ...
    flag_rerank_greenfield = "OFF"
    flag_parallel = "ON"
    flag_greenfield_method ="TWO"
    seed = 747
    nbr_cv_fold = 10
    p_src_code = "/scratch/mblab/dabid/netprophet/code_NetProphet_2.0/"
    flag_microarray = "ON"
  } else if (opt$flag_debug == "OFF"){
  ...
  quit(status=generate_lasso_net(p_in_target=p_in_target
                                 , p_in_reg=p_in_reg
