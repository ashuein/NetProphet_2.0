combine_networks = function(p_in_train_binding
  ...
  if (length(colnames(df_in_train_de)) > 3){
    df_in_train_de = unlist(df_in_train_de)
  } else{
  ...
  if (length(colnames(df_in_train_bart)) > 3){
    df_in_train_bart = unlist(df_in_train_bart)
  } else{
  ...
  if (length(colnames(df_in_test_de)) > 3){
    df_in_test_de = unlist(df_in_test_de)
  } else{
  ...
  if (length(colnames(df_in_test_bart)) > 3){
    df_in_test_bart = unlist(df_in_test_bart)
  } else{
    test_bart = df_in_test_bart[3]
    colnames(test_bart) = "bart"
  }
  
  if (in_model == "netprophet"){
    # train the model
    train_interact = train_lasso * train_de * train_bart
    colnames(train_interact) = "interact"
    df_train_lasso_de_interact = data.frame(lasso=abs(train_lasso), de=abs(train_de), bart=abs(train_bart), interact=abs(train_interact))
    ...
    # test the model
    test_interact = test_lasso*test_de*test_bart
    colnames(test_interact) = "interact"
    df_test_lasso_de_interact = data.frame(lasso=abs(test_lasso), de=abs(test_de), bart=abs(test_bart), interact=abs(test_interact))
    ...
    if (!is.null(p_in_train_reg) & !is.null(p_in_test_reg)){
...
if (sys.nframe() == 0){
  ...
  p_in_train_bart = make_option(c("--p_in_train_bart"), type="character", help="path of bart network for training")
  ...
  p_in_test_bart = make_option(c("--p_in_test_bart"), type="character", help="path of bart network testing")
  ...
  opt_parser = OptionParser(option_list=list(p_in_train_binding, p_in_train_lasso, p_in_train_de, p_in_train_bart
                                             ...
                                             , sep, p_in_train_reg, p_in_test_reg
                                             ))
  
  opt = parse_args(opt_parser)
  
  quit(status=combine_networks(
